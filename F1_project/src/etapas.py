...
def exibir_detalhes_etapa(etapa):
    ...
    print(f"Local: {etapa.get('local', 'Não especificado')}")
...
def adicionar_etapa():
    ...
    local = input("Local da etapa: ")

    etapas = carregar_etapas()
    etapas.append({"nome": nome, "data": data, "local": local})
...
def editar_etapa():
    ...
    if opcao.isdigit() and 1 <= int(opcao) <= len(etapas):
        ...
        etapas[idx]['local'] = input(f"Novo local ({etapas[idx]['local']}): ") or etapas[idx]['local']
